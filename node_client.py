...
async def test_signup():
    ...
    password_hash = nacl.hash.sha512(
        "test password".encode(), encoder=nacl.encoding.RawEncoder
    )
    signup_message = dict(
    ...
    print(
        "signup <-",
        await make_wrapped_http_request(
            aiohttp_client_session, packer, unpacker, SIGNUP_URL, signup_message
        ),
    )
...
async def test_latest():
    ...
    print(
        "login <-",
        await make_wrapped_http_request(
            aiohttp_client_session, packer, unpacker, LOGIN_URL, login_message
        ),
    )
    print(
        "latest <-",
        await make_wrapped_http_request(
            aiohttp_client_session, packer, unpacker, LATEST_URL, ""
        ),
    )
...
async def test_alerts():
    ...
    print(
        "alerts <-",
        await make_wrapped_http_request(
            aiohttp_client_session, packer, unpacker, ALERTS_URL, {"alerts": []}
        ),
    )
...
async def test_race():
    ...
    print(
        "raced 10, ", len([x for x in await asyncio.gather(*futures) if x]), "succeeded"
    )
